    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // Block unverified email users (skip in dev)
        ...
        try {
          ...
          if (result.rows.length === 0) {
            ...
            if (emailCheck.rows.length > 0) {
              ...
              await sql`
                INSERT INTO users (email, name, google_id, last_login, email_verified, email_verified_at)
                VALUES (${user.email}, ${user.name}, ${account.providerAccountId}, NOW(), true, NOW())
              `
            }
          } else {
            await sql`
